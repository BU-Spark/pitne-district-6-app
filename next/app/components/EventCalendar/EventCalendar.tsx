  ...
  const formatEventDate = (event: EventApi) => {
    ...
    if (event.allDay) {
      return end && start.toDateString() !== end.toDateString()
        ? `${start.toLocaleDateString(lang, fullDate)} - ${end.toLocaleDateString(lang, fullDate)}`
    ...
    if (end) {
      if (start.toDateString() === end.toDateString()) {
        /* same-day event */
        return `${start.toLocaleDateString(lang, fullDate)}, ${fmtTime(start)} - ${fmtTime(end)}`;
      }
      /* multi-day, timed event */
      return `${start.toLocaleDateString(lang, shortDate)}, ${fmtTime(start)} - ${end.toLocaleDateString(lang, shortDate)}, ${fmtTime(end)}`;
    }
        ...
        <>
          ...
          <div className={styles.eventPopup}>
            ...
            <div className={styles.popupActions}>
              <button className={styles.addToCalendarButton} onClick={() => handleAddToCalendar(selectedEvent)}>
